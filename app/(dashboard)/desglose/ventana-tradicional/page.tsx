...
import {
	...
	deletePendingBreakdowns,
	createInitialBreakdown,
	updateBreakdownItems,
	deleteWindowBreakdown,
	markAsProduction,
	getWindowBreakdownById,
	type WindowBreakdownItem,
} from "@/actions/window-breakdown-actions";
import {
	getCompanySettings,
...
export default function VentanaTradicionalPage() {
	...
	const [breakdownsPendientes, setBreakdownsPendientes] = useState<
		WindowBreakdown[]
	>([]);
	...
	const [currentBreakdownId, setCurrentBreakdownId] = useState<string | null>(
		null,
	);
	const [mostrarModalGuardado, setMostrarModalGuardado] =
		useState<boolean>(false);
	const [breakdownGuardado, setBreakdownGuardado] =
		useState<WindowBreakdown | null>(null);
	const [companySettings, setCompanySettings] =
		useState<CompanySettings | null>(null);
	const [mostrarConfirmarEliminarTrad, setMostrarConfirmarEliminarTrad] =
		useState<boolean>(false);
	const [breakdownIdAEliminarTrad, setBreakdownIdAEliminarTrad] = useState<
		string | null
	>(null);
		...
		if (editId) {
			const loadBreakdownForEdit = async () => {
				...
				if (breakdown) {
					...
					setResultados(
						breakdown.items.map((item: WindowBreakdownItem) => ({
							id: item.id,
							ancho: item.ancho,
							alto: item.alto,
	...
	const guardarDatosCliente = async () => {
		...
		try {
			...
			const pendings = await getPendingBreakdowns(
				nombreCliente,
				nombreTecnico,
				"TRADICIONAL",
			);
			...
			const result = await createInitialBreakdown(
				"TRADICIONAL",
				nombreCliente,
				nombreTecnico,
			);
			...
			if (result.success) {
				...
				setTimeout(() => {
					const inputAncho = document.getElementById(
						"ancho",
					) as HTMLInputElement;
					inputAncho?.focus();
				}, 100);
	...
	const parseFraction = (value: string): number => {
		...
		for (const part of parts) {
			if (part.includes("/")) {
				...
				const [numerator, denominator] = part
					.split("/")
	...
	const calcular = async () => {
		...
		if (!nombreCliente) {
			alert(
				"Por favor ingresa el nombre del cliente antes de agregar ventanas",
			);
			setTimeout(() => {
				const inputCliente = document.getElementById(
					"cliente",
		...
		if (!nombreTecnico) {
			alert(
				"Por favor ingresa el nombre del técnico antes de agregar ventanas",
			);
			setTimeout(() => {
				const inputTecnico = document.getElementById(
					"tecnico",
	...
	const handleSeguirEditando = () => {
		...
		if (breakdownsPendientes.length > 0) {
			...
			setResultados(
				mostRecent.items.map((item: ImportedWindowBreakdownItem) => ({
					id: item.id,
					ancho: item.ancho,
					alto: item.alto,
					resCabRiel: item.resCabRiel || 0,
		...
		<div className="space-y-6">
					...
					<CardContent className="space-y-4">
							...
							{isClient && !datosGuardados && (
								<div className="border-b border-dashed pb-4">
									<p className="text-sm font-semibold mb-3">
										Información del Cliente (Presiona Enter para guardar)
									</p>
						...
						<div className="text-sm text-muted-foreground">
							...
							<ul className="list-disc list-inside space-y-1">
								...
								<li>
									2. Ingresa el nombre del técnico y presiona Enter (o clic en
									Guardar)
								</li>
								<li>
									3. Ingresa medidas en fracciones (ej: 14 1/4) o decimales (ej:
									14.25)
								</li>
								<li>
									4. Presiona Enter o click en Calcular para agregar ventana
								</li>
									...
									<TableBody>
										{resultados.map((resultado) => (
											...
											>
												...
												<TableCell className="font-semibold bg-blue-100">
													{decimalToFraction(resultado.ancho)}
												</TableCell>
												<TableCell className="font-semibold bg-green-100">
													{decimalToFraction(resultado.alto)}
												</TableCell>
												<TableCell className="text-xs">
													{decimalToFraction(resultado.resCabRiel)}
												</TableCell>
												<TableCell className="text-xs">
													{decimalToFraction(resultado.resLateral)}
												</TableCell>
												<TableCell className="text-xs">
													{decimalToFraction(resultado.resJambas)}
												</TableCell>
												<TableCell className="text-xs">
													{decimalToFraction(resultado.resCabAlfDiv)}
												</TableCell>
												<TableCell className="text-xs">
													{decimalToFraction(resultado.resVAnchoDiv)}
												</TableCell>
												<TableCell className="text-xs">
													{decimalToFraction(resultado.resVAltura)}
												</TableCell>
												<TableCell>
													<Button
														variant="destructive"
				...
				<Card className="border-yellow-500 bg-yellow-50">
					...
					<CardContent>
						<p className="text-sm text-yellow-700 mb-4">
							Estos desgloses están guardados pero aún no han sido impresos. Haz
							clic para continuar editándolos.
						</p>
						<div className="space-y-3">
							{todosPendientes.map((breakdown) => (
								<div
									...
									onClick={() => {
										setResultados(
											breakdown.items.map(
												(item: ImportedWindowBreakdownItem) => ({
													id: item.id,
													ancho: item.ancho,
													alto: item.alto,
													resCabRiel: item.resCabRiel || 0,
									...
									<div className="flex justify-between items-start">
										<div>
											...
											<p className="text-sm text-muted-foreground">
												{breakdown.technicianName &&
													`Técnico: ${breakdown.technicianName} | `}
												Creado:{" "}
												{new Date(breakdown.createdAt).toLocaleString("es-DO")}
											</p>
										</div>
									...
									<div className="mt-2">
										...
										<div className="flex flex-wrap gap-1">
											{breakdown.items
												.slice(0, 4)
												.map(
													(item: ImportedWindowBreakdownItem, idx: number) => (
														<span
															key={idx}
															className="bg-yellow-100 px-2 py-1 rounded text-xs"
														>
															#{item.id}: {decimalToFraction(item.ancho)} x{" "}
															{decimalToFraction(item.alto)}
														</span>
													),
												)}
			...
			{!mostrarPendientes && todosPendientes.length > 0 && (
				<div className="fixed bottom-4 right-4 z-50">
					<Button
						...
						size="lg"
					>
						⚠️ {todosPendientes.length} Pendiente
						{todosPendientes.length !== 1 ? "s" : ""}
					</Button>
				</div>
			)}
			...
			{mostrarImpresion && resultados.length > 0 && (
				<>
					...
					<div
						id="printable-area-tradicional"
						className="font-mono text-sm w-[80mm] p-2 bg-white text-black mx-auto"
					>
						{/* Header */}
						<div className="text-center mb-3">
							<Image
								src={
									...
									companySettings.companyLogo.length > 0
										? companySettings.companyLogo
										: "/logo.png"
								}
								alt="Logo"
							...
							<h1 className="font-bold text-lg uppercase">
								{companySettings?.companyName || "FacturaDO"}
							</h1>
							{companySettings?.companyRnc && (
								<p className="text-xs">RNC: {companySettings.companyRnc}</p>
							)}
							{companySettings?.companyAddress && (
								<p className="text-xs">{companySettings.companyAddress}</p>
							)}
							{companySettings?.companyPhone && (
								<p className="text-xs">Tel: {companySettings.companyPhone}</p>
							)}
							<p className="text-xs font-bold">Desglose Ventana Tradicional</p>
							<p className="text-xs">
								Fecha: {isClient && new Date().toLocaleDateString("es-DO")}
							</p>
							<p className="text-xs">
								Hora:{" "}
								{isClient &&
						...
						<div className="mb-2 text-xs">
							{currentBreakdownId && (
								<p>
									<strong>Desglose #:</strong> {currentBreakdownId.slice(0, 8)}
								</p>
							)}
							{nombreCliente && (
								<p>
							...
							<p>
								<strong>Fecha:</strong> {new Date().toLocaleDateString("es-DO")}
							</p>
							<p>
								<strong>Total Ventanas:</strong> {resultados.length}
							</p>
						</div>
							...
							<tbody>
								{resultados.map((resultado) => (
									...
									>
										...
										<td className="py-1 text-center bg-blue-50">
											{decimalToFraction(resultado.ancho)}
										</td>
										<td className="py-1 text-center bg-green-50">
											{decimalToFraction(resultado.alto)}
										</td>
										<td className="py-1 text-center">
											{decimalToFraction(resultado.resCabRiel)}
						...
						<div className="border-t border-dashed border-black mt-3 pt-2 text-center">
							<p className="text-[10px] italic">
								Generado por {companySettings?.companyName || "FacturaDO"} -
								Desglose Ventana Tradicional
							</p>
						</div>
					</div>
				</>
			...
			<Dialog
				open={mostrarReinicioDialog}
				onOpenChange={setMostrarReinicioDialog}
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Ventanas Pendientes de Edición</DialogTitle>
						<DialogDescription>
							Se encontraron {breakdownsPendientes.length} desglose(s)
							pendiente(s) sin imprimir para {nombreCliente} con técnico{" "}
							{nombreTecnico}.
						</DialogDescription>
					</DialogHeader>
					<div className="py-4">
							...
							{breakdownsPendientes.map((breakdown) => (
								<div key={breakdown.id} className="text-sm p-2 border rounded">
									<p className="font-medium">
										{breakdown.totalWindows} ventana(s)
									</p>
									<p className="text-xs text-muted-foreground">
										Creado:{" "}
										{new Date(breakdown.createdAt).toLocaleString("es-DO")}
					...
					<DialogFooter>
						...
						<Button variant="destructive" onClick={handleEliminarPendientes}>
							Eliminar
						</Button>
						<Button onClick={handleSeguirEditando}>Seguir Editando</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
			...
			<Dialog
				open={mostrarModalGuardado}
				onOpenChange={setMostrarModalGuardado}
			>
				<DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						<DialogTitle>Desglose Guardado - Vista Previa</DialogTitle>
						<DialogDescription>
							Revisa la vista previa del desglose antes de imprimir o mandar a
							producción
						</DialogDescription>
					</DialogHeader>
					...
					{breakdownGuardado && (
						<div className="space-y-4">
							...
							<div className="border rounded-lg p-4 bg-gray-50">
								<div className="font-mono text-sm w-[80mm] p-2 bg-white text-black mx-auto">
									...
									<div className="text-center mb-3">
										...
										{companySettings?.companyRnc && (
											<p className="text-xs">
												RNC: {companySettings.companyRnc}
											</p>
										)}
										{companySettings?.companyAddress && (
											<p className="text-xs">
										...
										{companySettings?.companyPhone && (
											<p className="text-xs">
												Tel: {companySettings.companyPhone}
											</p>
										)}
									...
									{/* Información del pedido */}
									<div className="mb-2 text-xs">
										{breakdownGuardado.id && (
											<p>
												<strong>Desglose #:</strong>{" "}
												{breakdownGuardado.id.slice(0, 8)}
											</p>
										)}
										{breakdownGuardado.clientName && (
											<p>
												<strong>Cliente:</strong> {breakdownGuardado.clientName}
											</p>
										)}
										...
										<p>
											<strong>Total Ventanas:</strong>{" "}
											{breakdownGuardado.totalWindows}
										</p>
									</div>
									...
									<table className="w-full mb-2 text-xs">
										<thead>
											<tr className="border-b border-black">
												<th className="text-left py-1 w-6">Fab</th>
												<th className="text-center py-1 w-8">No</th>
												<th className="text-center py-1 w-12 bg-blue-100">
													Ancho
												</th>
												<th className="text-center py-1 w-12 bg-green-100">
													Alto
												</th>
												<th className="text-center py-1">Cab/R</th>
												<th className="text-center py-1">Lat</th>
										...
										<tbody>
											{breakdownGuardado.items.map((item) => (
												...
												>
													...
													<td className="py-1 text-center bg-blue-50">
														{decimalToFraction(item.ancho)}
													</td>
													<td className="py-1 text-center bg-green-50">
														{decimalToFraction(item.alto)}
													</td>
													<td className="py-1 text-center">
														{decimalToFraction(item.resCabRiel || 0)}
													</td>
													<td className="py-1 text-center">
														{decimalToFraction(item.resLateral || 0)}
													</td>
													<td className="py-1 text-center">
														{decimalToFraction(item.resJambas || 0)}
													</td>
													<td className="py-1 text-center">
														{decimalToFraction(item.resCabAlfDiv || 0)}
													</td>
													<td className="py-1 text-center">
														{decimalToFraction(item.resVAnchoDiv || 0)}
													</td>
													<td className="py-1 text-center">
														{decimalToFraction(item.resVAltura || 0)}
													</td>
												</tr>
											))}
										</tbody>
									...
									<div className="text-xs">
										<p className="font-bold">
											Total Ventanas: {breakdownGuardado.totalWindows}
										</p>
									</div>
							...
							<DialogFooter className="flex gap-2">
								...
								<Button onClick={handleModalCerrar} variant="outline">
									Cancelar
								</Button>
							</DialogFooter>
						</div>
			...
			<Dialog
				open={mostrarConfirmarEliminarTrad}
				onOpenChange={setMostrarConfirmarEliminarTrad}
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>¿Eliminar desglose?</DialogTitle>
						<DialogDescription>
							¿Estás seguro de que deseas eliminar este desglose? Esta acción no
							se puede deshacer.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
