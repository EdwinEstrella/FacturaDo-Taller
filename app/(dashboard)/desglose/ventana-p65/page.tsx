...
export default function VentanaP65Page() {
    ...
    const calcular = async () => {
        ...
        if (altoValue > 0 && anchoValue > 0) {
            // Des. Cab/Riel: 1.5 (1 1/2) al ancho
            const desCabRiel = 1.5
            const resCabRiel = anchoValue - desCabRiel

            // Des. Lateral: 1/8 (0.125) al alto
            const desLateral = 0.125
            const resLateral = altoValue - desLateral

            // Des. Jambas: 2 1/8 (2.125) al alto
            const desJambas = 2.125
            const resJambas = altoValue - desJambas

            // Cab/alf: 1 3/8 (1.375) al ancho ORIGINAL, dividido entre 2
            const desCabAlf = 1.375
            const resCabAlf = anchoValue - desCabAlf
            const resCabAlfDiv = resCabAlf / 2

            // Des. V. Ancho: 6.5 (6 1/2) al ancho ORIGINAL, dividido entre 2
            const desVAncho = 6.5
            const resVAncho = anchoValue - desVAncho
            const resVAnchoDiv = resVAncho / 2

            // V. Altura: 4.75 (4 3/4) al alto
            const desVAltura = 4.75
            const resVAltura = altoValue - desVAltura
    ...
    const handleImprimir = async () => {
        ...
        try {
            ...
            // Limpiar localStorage después de imprimir exitosamente
    ...
    return (
        <div className="space-y-6">
                                    ...
                                    <TableBody>
                                        {resultados.map((resultado) => (
                                            ...
                                            >
                                                ...
                                                <TableCell className="font-semibold">{decimalToFraction(resultado.resCabRiel)}</TableCell>
                                                <TableCell className="font-semibold">{decimalToFraction(resultado.resLateral)}</TableCell>
                                                <TableCell className="font-semibold">{decimalToFraction(resultado.resJambas)}</TableCell>
                                                <TableCell className="font-semibold">{decimalToFraction(resultado.resCabAlfDiv)}</TableCell>
                                                <TableCell className="font-semibold">{decimalToFraction(resultado.resVAnchoDiv)}</TableCell>
                                                <TableCell className="font-semibold">{decimalToFraction(resultado.resVAltura)}</TableCell>
                                                <TableCell>
                                                    <Button
                                                        variant="destructive"
                                                        size="sm"
            ...
            {/* Botón para mostrar pendientes si está oculto */}
            {!mostrarPendientes && todosPendientes.length > 0 && (
